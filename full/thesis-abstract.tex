...
For part (b), we propose a set of algorithms that precompute backup multicast trees to be used after a link failure. 
Each algorithm aims to minimize end-to-end packet loss and delay but each uses different optimization criteria to achieve this goal: minimizing control overhead, 
minimizing \xxn{the maximum number of flows impacted by the ``next'' link failure (\mfs)}, and minimizing \xxn{the maximum number of sink nodes impacted by the ``next'' link failure (\mds)}.
