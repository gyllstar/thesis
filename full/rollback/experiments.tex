...
First, we perform this simulation using \er graphs with fixed link costs.  The message overhead results are shown in Figure \ref{fig:many-fixed}(a) for $p=0.05$ and $n=100$. 
...
In the next three simulations we evaluate our algorithms over graphs with changing link costs. We introduce link cost changes between the time \bad is compromised and when \bad is discovered 
(e.g., during $[t',t_b]$). 
...
%Each algorithm using poisoned reverse is vulnerable to routing loops larger than $2$ that originate from link cost changes,
...
% \seconds+{\textsc PR} and \purges+{\textsc PR} use poisoned reverse to address routing loops resulting from link cost changes.  Thus both algorithms (and \cprs+{\textsc PR})
...
In cases where link costs change, we found that \purge using poisoned reverse is only slightly worse than \cprs+{\textsc PR}. % with poisoned reverse. 
