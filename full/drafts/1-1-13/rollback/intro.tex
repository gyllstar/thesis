...
%\begin{framed}
%\xxxxn{mention that distance vector needs to consider control (storage and maintainence) overhead.  The lack of control state stored at a given node can be a benefit to some applications.
%For example, military applications may not want all control state to be stored on a given node.}
%\end{framed}

%In this chapter, we design, implement, and evaluate algorithms that allow for recovery from failed nodes in a distributed system.
...
%For example, in 2009 \cite{Google}, a routing error forced Google to redirect its traffic through Asia, causing congestion that left many Google services unreachable. 
