...
%In Section \ref{subsec:merge}, we stated that \merge can speed \pcnt processing because, with \merges, \pcnt can install and query fewer flow entries to achieve similar accuracy results
...
%outports with a single flow entry. As a result, when \pcnt copies the flow entry, $e_i$, of a monitored flow, $f_i$, for tagging and counting purposes, \pcnt indirectly monitors all other 
...
\begin{figure*}[t]
  \begin{center}
    \subfigure[Loss estimates as function of \pcnt window size.]{\label{fig:pcount-loss-windows}\includegraphics[scale=0.31]{figs/pcount-loss-windows.pdf}}
    \subfigure[Loss estimates as function of number of packets.]{\label{fig:pcount-loss-pkts}\includegraphics[scale=0.31]{figs/pcount-loss-pkts.pdf}}
    \subfigure[Processing time, with $95\%$ confidence intervals, as a function of number of monitored flows.]{\label{fig:pcount-dtime}\includegraphics[scale=0.31]{figs/pcount-dtime-w5-j4000.pdf}}
  \end{center}
...
%Because at least a single flow entry is needed for each unique set of outports to ensure correct forwarding, the optimal solution cannot be less than the lower bound. 
...
%Because Mininet lacks performance fidelity we use flow entry install times from a real OpenFlow switch (the Pronto 3290 switch running the Indigo 2012.09.07 firmware)
...
%real OpenFlow hardware switches.  Instead, we apply flow entry installation times reported in the literature \cite{Ferguson13} measured using the Pronto 3290 switch running the 
...
%and more candidate backup tree flow entries that can be consolidated using \merges.  Thus, 
...
%The ratio starts at $1$ when $m=1$ and increasing to a maximum of $2.71$ when $m=28$. As more $m$ grows, on average there are both more backup tree flow entries that can be consolidate using
...
\begin{figure*}[t]
  \begin{center}
    \subfigure[Number of control messages to activate backup trees.]{\label{fig:msgs57}\includegraphics[scale=0.31]{figs/msgs-ieee57.pdf}}
	\subfigure[Total time to install all backup tree flow entries.]{\label{fig:install-time57}\includegraphics[scale=0.31]{figs/install-time-ieee57.pdf}}
	\subfigure[\pres: mean number of pre-installed rules per switch.]{\label{fig:preinstall57}\includegraphics[scale=0.31]{figs/preinstall-ieee57.pdf}}
	...
	\subfigure[\posts: number of stale flow entries resulting from link failure. ]{\label{fig:reactive-garbage57}\includegraphics[scale=0.31]{figs/garbage-ieee57.pdf}}
    \subfigure[\pres: number of stale flow entries resulting from link failure. ]{\label{fig:preinstall-garbage57}\includegraphics[scale=0.31]{figs/preinstall-garbage-ieee57.pdf}}
  \end{center} 
