...
%(Section \ref{subsec:openflow}) is made available since $k'$, rather than $k$, tagging and counting flow entries are installed at $u$ and $d$, respectively.
%results from fewer having sending fewer statistic queries Detection time improves $u$ must process one statistics query per monitored flow; reduce forwarding table size at $u$ and $d$ since, for each monitored flow, a copy of the corresponding flow entry is made at $u$ and $d$,
