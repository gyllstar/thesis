...
%can only support a limited number of flow entries.
%Additionally, \merge can yield faster recovery when backup trees are installed using \posts.  Fewer backup tree flow entries translates to fewer control messages to activate backup trees.
...
%In contrast, \merge leverages that $T_1$ and $T_2$ both use the same outgoing links at $c$ and $d$ to replace the two separate flow entries at $c$ and $d$, created under \bases, with a single 
...
A tag is a globally unique Ethernet address \merge writes in a packet header's {\tt dl\_dst} field (i.e., the Ethernet destination address).  When possible, \merge flow entries use 
...
%to two separate flow entries that would be created with \bases).  %After marking the edges used by $T_1$ and $T_2$, \merge
...
In the second round of BFSs, \merge consolidates flow entries among the other backup trees for $l$
...
%\merge attempts to reuse flow entries of primary trees already installed in the network that share forwarding behavior with a backup tree. 
%First, \merge attempts to reuse flow entries of primary trees already installed in the network that share forwarding behavior with a backup tree.  At nodes where this possible, no 
%flow entry needs to be installed.  At the remaining nodes, % At nodes where this is not possible,
...
%each multicast tree (primary tree and backup trees) using $v$ to the flow entry installed at $v$. When a link, $l$, fails the garbage collection routine determines 
...
%With \merges, garbage collection adds an additional check before removing any candidate stale flow entry, $c_v$, found using the procedure described in the previous paragraph.  
...
%\merge complicates garbage collection because it is no longer the case that an installed flow entry is used by a single multicast tree (e.g., multiple primary trees
%can share the same flow entry).  Garbage collection must now consider that a different set of primary trees
%use flow entry, $e$, before $l$ fails versus after $l$ fails and backup trees $\hat{T}^l$ are installed.  To account for this, we modify the first step of garbage collection.  
%Rather than simply add $T_i$'s flow entry, $e_v$, at each $v$ to $E_r$, the garbage collection first checks if any newly installed tree (i.e., each tree in $\hat{T}^l$) reuses $e_v$. Then, we
...
Regarding (a), consider an arbitrary multicast tree (primary of backup tree), $T_i$. \merge creates at most one flow entry at any $v \in T_i$.  
...
%As an aside, \merges's do no harm property ensures that under \merges, garbage collection has fewer flow entries to remove than with \bases.  Intuitively, this 
%is reasonable because \merge seeks to reuse primary tree flow entries. 
...
\pcnt sends $k$ queries to $u$ to read the state of each flow entry generated by \pcnts. % generated flow entry and a single aggregate statistic query to $d$.

%a function of the number of statistic queries \pcnt sends. % to the switches of a monitored link.  
%Recall from Section \ref{subsec:pcnt}, when estimating packet loss of a link, $(u,d)$, \pcnt makes a copy of the flow entry at $u$ and $d$, for each flow \pcnt monitors. 
...
%As a mechanism to count packet loss, \pcnt makes a copy of the flow entry of each monitored flow where the flow entry copy upstream tags packets upstream and a second flow entry copy counts 
%tagged packets downstream.  Because with \merge the same flow entry, $e_i$, may be shared by multiple flows, all of these flows can be monitored at the cost of monitoring a single flow: 
