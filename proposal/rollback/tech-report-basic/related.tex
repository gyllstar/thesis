...
%Detecting misbehaving routers is a well-studied problem. The algorithms presented in \cite{Arini,Feam,Feldmann,Vishal02,Pad03,Paul02} can be used to 
%identify misbehaving node(s), which can then be input to our recovery algorithms. 
...
%Additionally, if we treat link cost change events that occur after the compromised node has been discovered as database transactions, we face a similar design decision as in  \cite{Liu98}: 
...
%We use diffusing computations differently. 
%\cpr uses diffusing computations to force all nodes to rollback to a checkpoint and the preprocessing procedure (Section \ref{subsec:preprocess}) uses diffusing computations to remove \bad as a destination. 
...
%Garcia-Lunes-Aceves \cite{JJ93} uses diffusing computations to ensure routing table updates do not cause loops. %loop-free distance-vector routing. 
%The diffusing computations serve to query other network nodes.  We use diffusing computations differently.  Our diffusing computations are
%used to perform an update at each network node: rollback to a chackepoint in the case of \cprs, remove \bad as a destination during preprocessing, and 
%remove \bad with \purges.
%each of our recovery algorithms use diffusing computations to remove compromised node(s) as a destination and 
%\purge uses a diffusing computation to globally remove all false routing state.
